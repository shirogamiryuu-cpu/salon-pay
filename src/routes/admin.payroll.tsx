  ...
  const generateMut = useMutation({
    mutationFn: async (f: PayrollForm) => {
      ...
      for (const [staffId, g] of groups.entries()) {
        ...
        await supabase
          .from("commission_entries")
          .update({ status: "included", payroll_item_id: item.id })
          .in("id", g.ids);
      }
      return run;
    },
  ...
  const markPaidMut = useMutation({
    mutationFn: async (runId: string) => {
      const { data: itemList } = await supabase
        .from("payroll_items")
        .select("id,staff_user_id,net_amount")
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Generate, approve, and pay out staff commissions.
          </p>
        </div>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                All pending commission entries in this period will be grouped by staff.
              </p>
            </div>
            <DialogFooter>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="text-right">
                        <div className="font-mono font-semibold">
                          MMK {Number(r.total_amount).toFixed(2)}
                        </div>
                        <Badge
                          variant={
                            r.status === "paid"
            ...
            {selectedRun && (
              <div className="flex gap-2">
                ...
                {runs?.find((r) => r.id === selectedRun)?.status === "draft" && (
                  <Button size="sm" onClick={() => approveMut.mutate(selectedRun)}>
                    <Check className="h-3.5 w-3.5 mr-1" /> Approve
                  </Button>
                )}
                {runs?.find((r) => r.id === selectedRun)?.status === "approved" && (
                  <Button size="sm" onClick={() => markPaidMut.mutate(selectedRun)}>
                    Mark paid
                  </Button>
                )}
              </div>
                  ...
                  return (
                    <div key={it.id} className="p-4 flex items-center justify-between text-sm">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {it.entries_count} sessions
                        </div>
                      </div>
                      <div className="font-mono font-semibold">
                        MMK {Number(it.net_amount).toFixed(2)}
                      </div>
                    </div>
                  );
                })}
